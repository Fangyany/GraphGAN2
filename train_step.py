...
def train(epoch, config, train_loader, net, loss, post_process, opt):
    ...
    for i, data in enumerate(train_loader):
        ...
        if d_steps_left > 0:
            ...
            losses_g = generator_step(data, out_rel, loss, g_loss_fn, discriminator, optimizer_g)
            g_steps_left -= 1 
        ...
        if num_iters % save_iters == 0 or epoch >= config["num_epochs"]:
            save_ckpt(net, optimizer_g, config["save_dir"], "generator", epoch)
...
def generator_step(data, out_rel, out, loss, g_loss_fn, discriminator, optimizer_g):
